function [boundary, interior, region,perfect, distanceinfo] = projection(A,cursor,a,b,c,positions,distanceinfo)
...
while ~isequal(size(checklist), [0, 1])
    ...
    for i = 1:26
        % points is the 27 nearest neighbor of the starting point
        points = zeros(27, 3);
        ...
        if inData && (distanceinfo{Point4, 3} == 0)
            ...
            if ~inChecklist
                ...
                isInterior = qualify(neighbors(i,:), a,b,c, distanceinfo, positions, Point4);
                if isInterior
